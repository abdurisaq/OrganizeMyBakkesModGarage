...
Made by Bakkes
https://github.com/bakkesmodorg/BakkesModLoadoutLib/tree/master
*/
template<typename A>
...
public:
	...
	{
		LOG("Loadout string: {}", hexString);
		std::vector<BYTE> decodedVector = base64_decode_bytearr(hexString);
		LOG("Decoded vector size: {}", decodedVector.size());
		//std::string decoded = base64_decode(hexString);
		//const char* data = decoded.c_str();
		if (decodedVector.empty()) {
			std::cerr << "Error: Failed to decode base64 input or input is empty." << std::endl;
			exit(1);
		}
